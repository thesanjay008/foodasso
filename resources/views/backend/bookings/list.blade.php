...
@section('content')
				...
				<div class="main-card mb-3 card">
					<div class="card-header-tab card-header">
						<div class="card-header-title font-size-lg text-capitalize font-weight-normal">{{trans('booking.title')}}</div>
						<div class="btn-actions-pane-right text-capitalize">
							<!--<a class="btn-wide btn-outline-2x mr-md-2 btn btn-outline-focus btn-sm" href="">{{trans('booking.add')}}</a>-->
						</div>
					</div>
					<div class="card-body">
						<div class="table-responsive">
							<table class="mb-0 table table-striped">
								<thead>
									<tr>
									 <th>#</th>
									 <th>{{ trans('booking.order_date') }}</th>
									 <th>{{ trans('booking.customer_name') }}</th>
...
<script>
	...
	$(document).ready(function(e) {
		...
		$(document).on('change','.status',function(){
			...
			var response = adminAjax('{{route("ajax_change_BookingStatus")}}', data);
	...
	function getData(){
		var data = new FormData();
		var response = adminAjax('{{route("ajax.booking.list")}}', data);
		if(response.status == '200'){
