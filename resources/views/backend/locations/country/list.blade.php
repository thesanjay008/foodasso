					...
					<div class="row">
						<div class="col-md-12">
							<div class="input-group">
								...
								<input id="searchBox" placeholder="Enter Country Title to Search" type="text" class="form-control">
							</div>
							<br/>
						</div>
						<div class="col-md-6">
...
@section('content')
			...
			<div class="page-title-heading">
				<div>
					<div class="page-title-head center-elem">
						...
						<span class="d-inline-block">{{ trans('country.heading') }}</span>
					</div>
				</div>
			</div>
				...
				<div class="page-title-subheading opacity-10">
					<nav class="" aria-label="breadcrumb">
						<ol class="breadcrumb">
							...
							<li class="breadcrumb-item"> <a>{{ trans('country.plural') }}</a></li>
						</ol>
					</nav>
				</div>
			</div>
	...
	<div class="main-card mb-3 card">
		<div class="card-header-tab card-header">
			<div class="card-header-title font-size-lg text-capitalize font-weight-normal">{{trans('country.title')}}</div>
			<div class="btn-actions-pane-right text-capitalize">
				...
				<a class="btn-wide btn-outline-2x mr-md-2 btn btn-outline-focus btn-sm" href="{{ route('countries.create') }}">{{trans('country.add')}}</a>
			</div>
		</div>
		<div class="card-body">
				...
				<table class="mb-0 table table-striped">
					<thead>
						<tr>
						 ...
						 <th>{{ trans('country.iso_code') }}</th>
						 <th>{{ trans('country.calling_code') }}</th>
...
<script>
	...
	var page = $('#filterBox #page').val();
		...
		$(document).on('change','.status',function(){
			...
			var response = adminAjax('{{route("ajax.change.country.status")}}', data);
	...
	function getData(page = 1){
		...
		var response = adminAjax('{{route("ajax.country.list")}}', data);
		if(response.status == '200'){			
	...
	function deleteThis(item_id = ''){
		...
		if(confirm("{{trans('common.delete_confirm')}}")){
			...
			var response = adminAjax('{{route("ajax.delete.country")}}', data);
			if(response.status == '200'){
				swal.fire({ type: 'success', title: response.message, showConfirmButton: false, timer: 1500 });
