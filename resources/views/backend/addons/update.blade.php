...
@section('content')
						...
						<div class="page-title-heading">
							<div>
								<div class="page-title-head center-elem">
									...
									<span class="d-inline-block">{{ trans('addon.update') }}</span>
								</div>
							</div>
						</div>
				...
				<div class="main-card mb-3 card">
					<div class="card-body">
						<form class="" action="javascript:void(0);" onsubmit="saveAddon()">
							<input name="_method" type="hidden" value="PUT">
							<div class="form-row">
								<div class="col-md-6">
									<div class="position-relative form-group">
										<label for="title" class="">{{ trans('addon.title') }}</label>
										<input type="text" id="title" placeholder="{{ trans('addon.placeholder.enter_title') }}" class="form-control" value="{{$addons->title}}">
							...
							<div class="form-row">
								<div class="col-md-3">
									<div class="position-relative form-group">
										...
										<input type="text" id="price" placeholder="{{ trans('addon.placeholder.enter_price') }}" class="form-control" value="{{$addons->price}}">
									...
									<div class="form-group">
					            		<div class="position-relative form-group">
						                  	...
						                  	<select class="form-control" id="addon_group_id">
						                        ...
						                        @if($addons->addon_group_id ==  $addon_grp->id) selected @endif>
						                        {{$addon_grp->title}}
						                      	</option>
						                    	@endforeach
						                 	 </select>
								...
								<div class="col-md-2">
									<div class="form-group @error('status') ? has-error : ''  @enderror">
										...
										<select  class="form-control" minlength="2" maxlength="255" id="status" name="status">
										  <option value="active" @if($addons->status ==  'active') selected @endif>{{trans('common.active')}}</option>
										  <option value="inactive" @if($addons->status ==  'inactive') selected @endif>{{trans('common.inactive')}}</option>
										</select>
